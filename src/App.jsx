...
function App() {
  ...
  useEffect(() => {
    ...
    const checkNotificationPermission = async () => {
      if (!window.__TAURI_INTERNALS__) {
        console.log("🌐 Running in browser, skipping Tauri notification check");
        return;
      }
      try {
  ...
  const fetchPrayerTimes = useCallback(async () => {
    ...
    console.log("🚀 fetchPrayerTimes started for:", selectedLocation.name);
    ...
    try {
      ...
      console.log("🚀 Prayer times data:", data ? "SUCCESS" : "FAILED");
      ...
      setIsLoading(false);
      console.log("🚀 fetchPrayerTimes finished successfully");

    } catch (error) {
      console.error("❌ fetchPrayerTimes error:", error);
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (nextPrayerTime) {
        ...
        // Notification logic
        if (prayerTimes && window.__TAURI_INTERNALS__) {
          const prayerOrder = ["Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"];
          for (const pName of prayerOrder) {
            if (prayerTimes[pName]) {
              const pTime = parseTime(prayerTimes[pName], timezone);
              const diff = Math.abs(currentTime.getTime() - pTime.getTime());
              
              const todayKey = `${getTodayDateString()}-${pName}`;
              
              if (diff < 30000 && !notifiedPrayers.current.has(todayKey)) {
                try {
                  sendNotification({
                    title: `${pName} Time`,
                    body: `It is time for ${pName} prayer.`,
                    icon: "icon"
                  });
                } catch (err) {
                  console.error("❌ Send notification error:", err);
                }
                notifiedPrayers.current.add(todayKey);
              }
            }
          }
        }
  ...
  console.log("🚀 Rendering state:", { isLoading, hasPrayerTimes: !!prayerTimes, error });

  if (isLoading) {
    return (
      <div className="load">
        <Loader size="large" color={settings.primaryColor} />
        <p style={{ marginTop: '10px', color: '#170939', fontSize: '12px', fontWeight: 'bold' }}>Loading Prayer Times...</p>
      </div>
    );
  }
    ...
    return (
      <div className="load">
        <div style={{ padding: '20px', textAlign: 'center' }}>
          <p style={{ color: '#d63031', fontWeight: 'bold', marginBottom: '10px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#636e72', marginBottom: '20px' }}>
            {error && error.includes("fetch") 
              ...
              : "Please try selecting your location again."}
          </p>
          <button 
